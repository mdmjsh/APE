...
public class TideCalculator {
    ...
    public static BigDecimal midDayTide(String place, String date) throws IOException {
        ...
        Request request = new Request.Builder()
                .url(String.format("https://dry-fjord-40481.herokuapp.com/tides/%s/%s", place, date))
                .build();

        try (Response e = new OkHttpClient.Builder().build().newCall(request).execute()) {
...
//                System.out.println(l);
        ...
        Value first = new Value(time(result[1].split(" ")[1]), new BigDecimal(result[1].split(" ")[2]));
        Value second = new Value(time(result[2].split(" ")[1]), new BigDecimal(result[2].split(" ")[2]));
        ...
        double proportionOfWayThrough = (double) since.toMillis() / (double) between.toMillis();
        BigDecimal sinceLevelChange = betweenLevels.multiply(new BigDecimal(proportionOfWayThrough));
        return first.second.add(sinceLevelChange).setScale(2, RoundingMode.CEILING);
    }

    private static LocalTime time(String time) {
        return LocalTime.of(parseInt(time.split(":")[0]), parseInt(time.split(":")[1]));
    }
